...
function printFeeds($user_feed,$candidate_fbid,$facebook){
	...
	foreach($user_feed['data'] as $feed){
		...
		if ($count < 4 && $feed['from']['id']==$candidate_fbid){
			echo "Posted by " . $feed['from']['name']. "on " . date('D, F jS Y g:ia', strtotime($feed['created_time'])) . "<br />"."<br />";
			if($feed['type']=='status' || $feed['type']=='link'){
				echo "<div id=\"facewrap\">";
				echo auto_link($feed['message']) . "</div>"."<br />";	
				echo "<br />";			
			}
			...
			{
				echo "<br />";
				echo "<img src= \"likef.png\" width=\"20px\" border= none>"."$nbsp"."$nbsp"."$nbsp".$feed['likes']['count'] . " people like this post.";
				echo "<br />";
